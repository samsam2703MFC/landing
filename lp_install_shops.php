...
try {
    ...
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS lp_shops (
            ...
            webshop_active TINYINT(1)   NOT NULL DEFAULT 0,
    ...
    // ── Picker webshop : les vraies boutiques dont le webshop est actif ──
    // Étiquette = ville (city). Remplissez webshop_url + lat/lng par boutique.
    $pdo->exec("UPDATE lp_shops SET webshop_active = 1 WHERE is_active = 1");

    echo '<p style="font-family:monospace;color:green">✓ Tables lp_shops + lp_shop_hours + lp_shop_services créées (8 boutiques, webshop_active=1). Supprimez ce fichier.</p>';
